...
class Node:
    ...
    def __lt__(self, other: Any):
        if not isinstance(other, Node):
            raise ValueError(f"Node < {other.__class__} !?")
    ...
    def n_nodes(self) -> int:
        ...
        if n > Node._max_nodes_ever:
            LOGGER.info(f"New max number of nodes: {n}")
            Node._max_nodes_ever = n
        return n
    ...
    @property
    def depth(self) -> int:
        stack: list[tuple[int, "Node"]] = [(1, self)]
        max_seen = 0
        while stack:
        ...
        if max_seen > Node._max_depth_ever:
            LOGGER.info(f"New max depth: {max_seen}")
            Node._max_depth_ever = max_seen
        return max_seen
    ...
    @staticmethod
    def connect_parent_to_children(parent: "Node", children: List["Node"]):
    ...
    def compute_topological_sort(self) -> list["Node"] | None:
...
class Tree:
    ...
    def compute_topological_sort(self) -> list[Node]:
    ...
    @staticmethod
    def create_from_file(file: str) -> "Tree":
        """
        ...
        return Tree.parse_raw_sexp(file, raw)
    ...
    def parse_raw_sexp(file: str, raw: str):
        ...
        for i, c in enumerate(raw):
            if c == '"' and Tree.is_this_a_quote(raw, i):
    ...
    def parse_node(
        ...
        node_types, rest = Tree.extract_tags(to_process)
        if len(node_types) != 1:
            LOGGER.warning(
...
class Forest:
    def __init__(self, trees: List[Tree], is_internal: List[bool]):
        if len(trees) != len(is_internal):
            raise ValueError(
                "Lists in Forest(trees, is_external) should have the same length!"
            )
    ...
    def create_from_files(
        ...
        sexp_files = helpers.Other.get_all_files(sexp_dir, file_extension="sexp")
        for path in tqdm.tqdm(sexp_files):
            print(path)
            trees.append(Tree.create_from_file(path))
            is_internal.append(Forest._is_sexp_internal(source_root_dir, path))
        return Forest(trees, is_internal)
