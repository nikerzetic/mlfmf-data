...
def do_all_for_library(library: str, library_dir: str, stage: int):
    ...
    if stage <= 2:
        kg_loc = helpers.Locations.knowledge_graph(library)
        if not os.path.exists(kg_loc):
            kg = KnowledgeGraph.create_from_definitions_file(
                library, helpers.Locations.definitions_pickled(library)
            )
            kg.dump(kg_loc)
        g_loc = helpers.Locations.knowledge_graph_pure(library)
...
def create_datasets(libraries: list[str], p_test: float, ps_body_to_keep: list[float]):
    for library in libraries:
        for p_body_to_keep in ps_body_to_keep:
            dataset_file = helpers.Locations.dataset(library, p_test, p_body_to_keep)
            ...
            kg = KnowledgeGraph.load(helpers.Locations.knowledge_graph(library))
...
def create_internal_cv_dataset(
    ...
    for lib, p_body, fold in itertools.product(
        ...
        lib_internal = helpers.Locations.library_name_to_internal(lib, n_folds, fold)
        dataset_file = helpers.Locations.dataset(lib, 0.2, p_body)
        internal_dataset_file = helpers.Locations.dataset(lib_internal, 0.2, p_body)
        internal_pure_kg = helpers.Locations.knowledge_graph_pure(lib_internal)
...
def do_all_and_datasets():
    ...
    create_datasets(
        [
            ...
            helpers.Locations.NAME_LEAN_TEST,
...
def do_efficient_all(libs: list[str]):
    pairs = [
        (helpers.Locations.SEXP_DIR_AGDA_TEST, helpers.Locations.NAME_AGDA_TEST),
